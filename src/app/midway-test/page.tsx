...
export default function MidwayTestPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto" role="main" aria-labelledby="midway-test-title">
      <h1 id="midway-test-title" className="text-2xl font-bold mb-6">MCPサーバー接続テスト</h1>
      <div className="mb-4">
        <p className="text-gray-700">
          テスト対象URL: <code className="bg-gray-100 px-2 py-1 rounded" aria-label="テスト対象URL">{`${MCP_SERVER_URL}/mcp/status`}</code>
        </p>
        <button
      ...
      {apiResponse && !loading && !error && (
        <div className="bg-green-50 border border-green-200 p-4 rounded" role="status" aria-live="polite">
          ...
          <div className="bg-white p-4 rounded border border-gray-200 overflow-auto">
            <pre className="text-sm" aria-label="MCPサーバーレスポンス">{JSON.stringify(apiResponse, null, 2)}</pre>
          </div>
        </div>
      )}
