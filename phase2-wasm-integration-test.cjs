...
// ES Moduleの動的インポートを使用
async function loadWasmProcessor() {
  const { wasmTypingProcessor } = await import('./src/typing/wasm-integration/WasmTypingProcessor.js');
  return wasmTypingProcessor;
}
...
async function testPhase2WebAssembly() {
  ...
  try {
    // WebAssemblyプロセッサの動的ロード
    const wasmTypingProcessor = await loadWasmProcessor();
